...
<br>
<div> Total Metalicos En inventario: <span class="text-xl text-teal-900 px-4 py-2 font-bold"><?php echo $this->model->get('sum(drums) as total','rm'," $filters and returnToClient = 1")->total ?></span> </div>
<div> Total Cuñetes En inventario: <span class="text-xl text-teal-900 px-4 py-2 font-bold"><?php echo $this->model->get('sum(barrels) as total','rm'," $filters and returnToClient = 1")->total ?></span> </div>